...
//                textView.setText(query)
    ...
    private fun performSearch(query: String) {
        ...
        if (query.length > 2){
            productList.clear()
            productList.addAll(searchResultHandleList.filter { it.title?.lowercase()!!.contains(query.lowercase()) })
        }else {
            resetSearch()
        }
    ...
    private fun initializationOfCategory() {
        ...
        categoriesRv.layoutManager = LinearLayoutManager(activity,LinearLayoutManager.HORIZONTAL,
            false)
        // ProduceList adapter
        ...
        // View Model
        categoryListViewModel = ViewModelProviders.of(activity).get(CategoryListViewModel::class.java)
